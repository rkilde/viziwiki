...
// markup (wiki-hero-*, wiki-overview-*, wiki-infobox-*), so everything renders
// identically to the live site and tracks the master format. Every canonical
// element + optional add-on is represented. Editable text is contenteditable;
// +add / ×remove call back to the parent (React) via P()/A(). No pp-* copies.
...
const AFFORDANCE = `
  ...
  .pe-rm{ border:1px solid rgba(120,120,140,.35); background:rgba(255,255,255,.6); color:rgba(90,90,110,.7); }
...
function infoboxHTML(ib: NonNullable<InfoboxDoc>): string {
  return `<aside class="wiki-infobox">
    <div class="wiki-infobox-header">
      ...
      ${ib.sublabel != null
        ? `<div class="wiki-infobox-label" style="margin-top:4px">${ce('overview.infobox.sublabel', ib.sublabel)}${rm('rmSublabel')}</div>`
    ...
    ${ib.badge != null
      ? `<div class="pe-wrap" style="padding:8px 16px 0"><span class="wiki-infobox-badge">${ce('overview.infobox.badge', ib.badge)}</span>${rm('rmBadge')}</div>`
      : add('addBadge', '+ badge', 'padding:8px 16px 0')}
    <div class="pe-addline" style="padding:6px 16px 14px"><button class="pe-add" onclick="A('addRow')">+ row</button>${rm('rmInfobox')}</div>
  </aside>`;
}
